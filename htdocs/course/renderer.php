...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function course_section_cm($course, &$completioninfo, cm_info $mod, $sectionreturn, $displayoptions = array()) {
        ...
        if (!empty($cmname)) {
            ...
            //MY RANKING BUTTONS
            if ($mod->modfullname != 'Quiz') {
                //seems equal sign is not working
            }else{
                $url = new moodle_url('/quiz_grade/report/index.php');

                $mod_id = $mod->id; //$mod->get_formatted_name(); //$mod->url;
                $item_name = $mod->get_formatted_name();
                //$item_name = 'QUIZ';

                //get necessary id's in DB to check grade items
                /*$params = array('itemname'=>$item_name);
                $sql = "SELECT *
                          FROM {grade_items}
                         WHERE itemname = :itemname";
                        
                $recorditem = $DB->get_records_sql($sql, $params);

                foreach($recorditem as $id => $record) {
                    $sort_id = $record->id;
                    $item_id = $record->iteminstance;
                }*/

                //get necessary id's in DB to check grade items
                $params = array($mod_id, 'mod');
                $sql = "SELECT gi.id
                        FROM {grade_items} gi
                        JOIN {course_modules} cm ON cm.instance = gi.iteminstance
                        WHERE cm.id = ? AND gi.itemtype = ?";   
                ...
                //change spaces if name have spaces
                $item_name = urlencode($item_name);
 
                $output .= html_writer::start_tag('div');
                $output .= html_writer::start_tag('form', array('action' => $url)); 
                //$output .= html_writer::tag('span', ' ('. $mod_id.')'); 
                $output .= html_writer::tag('input', '', array('type' => 'hidden', 'name' => 'itemid', 'value' => $item_id));     
                $output .= html_writer::tag('input', '', array('type' => 'hidden', 'name' => 'id', 'value' => $course->id));    
                $output .= html_writer::tag('input', '', array('type' => 'hidden', 'name' => 'sortid', 'value' => $sort_id));
                $output .= html_writer::tag('input', '', array('type' => 'hidden', 'name' => 'modid', 'value' => $mod_id));                  
                $output .= html_writer::tag('button', 'Ranking', array('type' => 'submit'));     
                $output .= html_writer::end_tag('form');   
                $output .= html_writer::end_tag('div');
            }
