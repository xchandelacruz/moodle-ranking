...
require_once($CFG->dirroot . '/quiz_grade/export/lib.php');
...
function grade_get_graded_users_select($report, $course, $userid, $groupid, $includeall) {
    ...
    $select = new single_select(new moodle_url('/quiz_grade/report/'.$report.'/index.php', array('id'=>$course->id)), 'userid', $menu, $userid);
    $select->label = $label;
...
function print_natural_aggregation_upgrade_notice($courseid, $context, $thispage, $return=false) {
    ...
    if (!has_capability('moodle/quiz_grade:manage', $context)) {
        return $html;
    }
...
function grade_print_tabs($active_type, $active_plugin, $plugin_info, $return=false) {
    ...
    foreach ($plugin_info as $plugin_type => $plugins) {
        ...
        if ($plugin_type == 'report') {
            $url = $CFG->wwwroot.'/quiz_grade/report/index.php?id='.$COURSE->id;
        }
...
function grade_get_plugin_info($courseid, $active_type, $active_plugin) {
    ...
    $count = 0;
    $active = '';
    $url_prefix = $CFG->wwwroot . '/quiz_grade/';
...
class grade_plugin_return {
    ...
    public function get_return_url($default, $extras=null) {
        ...
        $url = $CFG->wwwroot.'/quiz_grade/'.$this->type.'/'.$this->plugin.'/index.php';
        $glue = '?';
...
function grade_build_nav($path, $pagename=null, $id=null) {
    ...
    $PAGE->navbar->add($strgrades, new moodle_url('/quiz_grade/index.php', array('id'=>$COURSE->id)));
    ...
    switch ($path_elements[1]) {
        ...
        case 'report':
            ...
            if (!is_null($id)) {
                $link = new moodle_url('/quiz_grade/report/index.php', $linkparams);
            }
    ...
    switch ($numberofelements) {
        ...
        case 4:
            if ($path_elements[2] == 'grader' AND $path_elements[3] != 'index.php') {
                $PAGE->navbar->add(get_string('pluginname', 'gradereport_grader'), new moodle_url('/quiz_grade/report/grader/index.php', $linkparams));
            }
    ...
    */
    ...
    public function get_reset_icon($element, $gpr, $returnactionmenulink = false) {
        ...
        if ($element['type'] != 'category' || $element['object']->aggregation != GRADE_AGGREGATE_SUM ||
                !has_capability('moodle/quiz_grade:manage', $this->context)) {
            return $returnactionmenulink ? null : '';
        }

        $str = get_string('resetweights', 'grades', $this->get_params_for_iconstr($element));
        $url = new moodle_url('/quiz_grade/edit/tree/action.php', array(
            'id' => $this->courseid,
            'action' => 'resetweights',
            'eid' => $element['eid'],
            'sesskey' => sesskey(),
    ...
    public function get_edit_icon($element, $gpr, $returnactionmenulink = false) {
        ...
        if (!has_capability('moodle/quiz_grade:manage', $this->context)) {
            if ($element['type'] == 'grade' and has_capability('moodle/quiz_grade:edit', $this->context)) {
        ...
        switch ($element['type']) {
            ...
            case 'courseitem':
                ...
                if (empty($object->outcomeid) || empty($CFG->enableoutcomes)) {
                    $url = new moodle_url('/quiz_grade/edit/tree/item.php',
                            array('courseid' => $this->courseid, 'id' => $object->id));
                } else {
                    $url = new moodle_url('/quiz_grade/edit/tree/outcomeitem.php',
                            array('courseid' => $this->courseid, 'id' => $object->id));
                }
                break;
            ...
            case 'category':
                $stredit = get_string('editverbose', 'grades', $strparams);
                $url = new moodle_url('/quiz_grade/edit/tree/category.php',
                        array('courseid' => $this->courseid, 'id' => $object->id));
                break;
            ...
            case 'grade':
                ...
                if (empty($object->id)) {
                    $url = new moodle_url('/quiz_grade/edit/tree/grade.php',
    ...
    public function get_hiding_icon($element, $gpr, $returnactionmenulink = false) {
        ...
        if (!has_capability('moodle/quiz_grade:manage', $this->context) and
            !has_capability('moodle/quiz_grade:hide', $this->context)) {
            return $returnactionmenulink ? null : '';
        }
        ...
        $url = new moodle_url('/quiz_grade/edit/tree/action.php', array('id' => $this->courseid, 'sesskey' => sesskey(), 'eid' => $element['eid']));
    ...
    public function get_locking_icon($element, $gpr) {
        ...
        if ($element['type'] == 'grade' && $element['object']->grade_item->is_locked()) {
            ...
            if (!has_capability('moodle/quiz_grade:manage', $this->context) and !has_capability('moodle/quiz_grade:unlock', $this->context)) {
                $action = '';
            } else {
            ...
            if (!has_capability('moodle/quiz_grade:manage', $this->context) and !has_capability('moodle/quiz_grade:lock', $this->context)) {
                $action = '';
            } else {
    ...
    public function get_calculation_icon($element, $gpr, $returnactionmenulink = false) {
        ...
        if ($type == 'item' or $type == 'courseitem' or $type == 'categoryitem') {
            ...
            if (!$object->is_external_item() and ($is_scale or $is_value)) {
                ...
                $url = new moodle_url('/quiz_grade/edit/tree/calculation.php', array('courseid' => $this->courseid, 'id' => $object->id));
                $url = $gpr->add_url_params($url);
                if ($returnactionmenulink) {
...
abstract class grade_helper {
    ...
    public static function get_info_manage_settings($courseid) {
        ...
        if ($courseid != SITEID && has_capability('moodle/quiz_grade:manage', $context)) {
            self::$managesetting['gradebooksetup'] = new grade_plugin_info('setup',
                new moodle_url('/quiz_grade/edit/tree/index.php', array('id' => $courseid)),
                get_string('gradebooksetup', 'grades'));
            self::$managesetting['coursesettings'] = new grade_plugin_info('coursesettings',
                new moodle_url('/quiz_grade/edit/settings/index.php', array('id'=>$courseid)),
    ...
    public static function get_plugins_reports($courseid) {
        ...
        foreach (core_component::get_plugin_list('gradereport') as $plugin => $plugindir) {
            ...
            if ($plugin === 'singleview' && !has_all_capabilities(array('moodle/quiz_grade:viewall',
                    'moodle/quiz_grade:edit'), $context)) {
                continue;
            }

            $pluginstr = get_string('pluginname', 'gradereport_'.$plugin);
            $url = new moodle_url('/quiz_grade/report/'.$plugin.'/index.php', array('id'=>$courseid));
            ...
            if (file_exists($plugindir.'/preferences.php')) {
                $url = new moodle_url('/quiz_grade/report/'.$plugin.'/preferences.php', array('id'=>$courseid));
    ...
    public static function get_info_scales($courseid) {
        ...
        if (has_capability('moodle/course:managescales', context_course::instance($courseid))) {
            $url = new moodle_url('/quiz_grade/edit/scale/index.php', array('id'=>$courseid));
    ...
    public static function get_info_outcomes($courseid) {
        ...
        $context = context_course::instance($courseid);
        $canmanage = has_capability('moodle/quiz_grade:manage', $context);
        $canupdate = has_capability('moodle/course:update', $context);
        if (!empty($CFG->enableoutcomes) && ($canmanage || $canupdate)) {
            $outcomes = array();
            if ($canupdate) {
                if ($courseid!=$SITE->id) {
                    $url = new moodle_url('/quiz_grade/edit/outcome/course.php', array('id'=>$courseid));
                    $outcomes['course'] = new grade_plugin_info('course', $url, get_string('outcomescourse', 'grades'));
                }
                $url = new moodle_url('/quiz_grade/edit/outcome/index.php', array('id'=>$courseid));
                $outcomes['edit'] = new grade_plugin_info('edit', $url, get_string('editoutcomes', 'grades'));
                $url = new moodle_url('/quiz_grade/edit/outcome/import.php', array('courseid'=>$courseid));
    ...
    public static function get_info_letters($courseid) {
        ...
        $context = context_course::instance($courseid);
        $canmanage = has_capability('moodle/quiz_grade:manage', $context);
        $canmanageletters = has_capability('moodle/quiz_grade:manageletters', $context);
        if ($canmanage || $canmanageletters) {
            ...
            self::$letterinfo = array(
                'view' => new grade_plugin_info('view', new moodle_url('/quiz_grade/edit/letter/index.php', array('id'=>$context->id)), get_string('view')),
                'edit' => new grade_plugin_info('edit', new moodle_url('/quiz_grade/edit/letter/index.php', $param), get_string('edit'))
            );
        } else {
    ...
    public static function get_plugins_import($courseid) {
        ...
        if (has_capability('moodle/quiz_grade:import', $context)) {
            foreach (core_component::get_plugin_list('gradeimport') as $plugin => $plugindir) {
                ...
                $url = new moodle_url('/quiz_grade/import/'.$plugin.'/index.php', array('id'=>$courseid));
            ...
            if ($CFG->gradepublishing && has_capability('gradeimport/xml:publish', $context)) {
                $url = new moodle_url('/quiz_grade/import/keymanager.php', array('id'=>$courseid));
    ...
    public static function get_plugins_export($courseid) {
        ...
        $exportplugins = array();
        $canpublishgrades = 0;
        if (has_capability('moodle/quiz_grade:export', $context)) {
            foreach (core_component::get_plugin_list('gradeexport') as $plugin => $plugindir) {
                ...
                $url = new moodle_url('/quiz_grade/export/'.$plugin.'/index.php', array('id'=>$courseid));
            ...
            if ($CFG->gradepublishing && $canpublishgrades != 0) {
                $url = new moodle_url('/quiz_grade/export/keymanager.php', array('id'=>$courseid));
