...
require_once($CFG->dirroot . '/quiz_grade/report/lib.php');
require_once($CFG->libdir.'/tablelib.php');
//require_once($CFG->dirroot . '/quiz_grade/report/grader/index.php');
...
class grade_report_grader extends grade_report {
    ...
    public function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null) {
        ...
        $this->baseurl = new moodle_url('index.php', array(
            'id'        => $this->courseid, 
            'itemid'    => $item_id
        ));
        ...
        $this->pbarurl = new moodle_url('/quiz_grade/report/grader/index.php', array('id' => $this->courseid));
    ...
    public function get_right_rows($displayaverages) {
        global $CFG, $USER, $OUTPUT, $DB, $PAGE, $item_id, $itemi, $courseid, $mod_id;
        ...
        foreach ($this->gtree->get_levels() as $key => $row) {
            ...
            foreach ($row as $columnkey => $element) {
                ...
                if ($type == 'filler' or $type == 'fillerfirst' or $type == 'fillerlast') {
                    ...
                    if ($element['object']->id == $this->sortitemid) {
                        if ($this->sortorder == 'ASC') {
                            $arrow = $this->get_sort_arrow('up', $sortlink);
                                //redirect again if not DESC order
                                $elemen_id = $element['object']->id;

                                if ($elemen_id == $item_id) {
                                    if (!empty($mod_id)) {
                                        redirect($CFG->wwwroot.'/quiz_grade/report/grader/index.php?id='.$courseid.'&itemid='.$item_id.'&sortitemid='.$item_id);
                                        $mod_id = 0;
                                    }
                                }
                        } else {
                    ...
                    $itemelement = $this->gtree->get_element_header($element);
                    ...
                    if (has_all_capabilities(array('gradereport/singleview:view', 'moodle/grade:viewall',
                        ...
                        $url = new moodle_url('/quiz_grade/report/singleview/index.php', array(
                            'id' => $this->course->id,
                            'item' => 'grade',
                    ...
                    //MY RANKING
                    /*if (strpos($itemelement, '>'.$item_name.'<') !== false) {
                        $headingrow->cells[] = $itemcell;
                        $itemi = $element['object']->id;
                        //$_session["item_name_holder"] = $item_name;
                    }*/
        ...
        $module = array(
            ...
            'fullpath'  => '/quiz_grade/report/grader/module.js',
    ...
    public function get_sort_arrows(array $extrafields = array()) {
        ...
        //for name arrows
        $firstlink = html_writer::link(new moodle_url($this->baseurl, array('itemid' => $item_id, 'sortitemid'=>'firstname')), $strfirstname);
        $lastlink = html_writer::link(new moodle_url($this->baseurl, array('itemid'=> $item_id, 'sortitemid'=>'lastname')), $strlastname);
        ...
        foreach ($extrafields as $field) {
            $fieldlink = html_writer::link(new moodle_url($this->baseurl,
                    array('itemid' => $item_id, 'sortitemid'=>$field)), get_user_field_name($field));
