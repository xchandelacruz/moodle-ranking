...
require_once($CFG->dirroot.'/quiz_grade/lib.php');
require_once($CFG->dirroot.'/quiz_grade/report/grader/lib.php');

$courseid      = required_param('id', PARAM_INT);        // course id
$item_id       = optional_param('itemid', 0, PARAM_INT);        // module name
$sort_id       = optional_param('sortid', 0, PARAM_ALPHANUM); //item id
$mod_id        = optional_param('modid', 0, PARAM_ALPHANUM); //check for first sorting
...
$PAGE->set_url(new moodle_url('/grade/report/grader/index.php', array(
    'id'         => $courseid, 
    'itemid'     => $item_id,
    'sortid'     => $sort_id 
)));
$PAGE->requires->css('/quiz_grade/report/grader/styles.css');
...
// User search
$url = new moodle_url('/quiz_grade/report/grader/index.php', array('id' => $courseid, 'itemid' => $item_id));
...
echo $renderer->user_search($url->out(false), $firstinitial, $lastinitial, $numusers, $totalusers, $report->currentgroupname);
